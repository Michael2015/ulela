...
class search_goods
{
	...
	{
		return array('sale' =>'销量','cpoint' =>'评分','price'=>'价格','new'=>'上架时间');
	}
	...
	public static function find($defaultWhere = '',$limit = 21,$isCondition = true)
	{
		...
		$goodsObj->fields   = 'go.id,go.name,go.sell_price,go.market_price,go.store_nums,go.img,go.sale,go.grade,go.comments,go.favorite,go.keywords, go.description'; //dyg_jzw 20161202添加keywords, description字段
			...
			else if(is_array($defaultWhere))
			{
				foreach($defaultWhere as $key => $val)
				{
					...
					if($key == 'category_extend')
					{
						...
						//dyg_jzw 20161213 隐藏分类无法搜索
						if ($val == 0)
						{
							$join[]  = "left join category_extend as ce on ce.goods_id = go.id";
							$where[] = "(ce.category_id <> 0 or ce.category_id is null)";
						}
					}
					//搜索词模糊
					else if($key == 'search')
