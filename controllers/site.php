...
class Site extends IController
{
    ...
    function index()
    {
        $this->index_slide = Api::run('getBannerList', 'pc');
        $this->index_slide_mobile = Api::run('getBannerList', 'mobile'); //dyg_jzw 20161212加入手机版首页幻灯片
        $this->redirect('index');
    }
    ...
    function pro_list()
    {
        $prop_id = IFilter::act(IReq::get('prop_id'),'int');//分类id
        $this->catId = IFilter::act(IReq::get('cat'),'int');//分类id
        $page = IFilter::act(IReq::get('page'),'int');//分页 //dyg_jzw 20160804 自动跳转

        if($this->catId == 0 && $prop_id == 0)
        {
        ...
        if($this->catId)
        {
            ...
            //获取分类商品
            $this->goodsObj = search_goods::find(array('category_extend' => $this->childId), 20);
            $this->resultData = $this->goodsObj->find();
        }
        if($prop_id)
        {
            $ticketObj  = new IQuery('prop as p');
            $ticketObj->join = "left join ticket as t on p.condition = t.id";
            $ticketObj->fields = 't.goods_ids,t.name,t.value,t.start_time,t.end_time';
            $ticketObj->where = 'p.id = '.$prop_id;
            $ticket_info = $ticketObj->find()[0];
            $this->ticket =  $ticket_info;
            $goods_id_str = $ticket_info['goods_ids'];
            if($goods_id_str)
            {
                $this->goodsObj = search_goods::find('go.id in ('.$goods_id_str.')', 20);
                $this->resultData = $this->goodsObj->find();
            }
        }
        //dyg_jzw 20160804 自动跳转
        if (count($this->resultData) === 1 && empty($page))
        {
            //如果分类下只有一个产品，则直接跳转到目标
            $this->redirect('products?id='.$this->resultData[0]['id']);
        }
        else
        {
    ...
    {
        $this->activity_id = IFilter::act(IReq::get('id'),'int');
        if($this->activity_id == '')
        {
            IError::show(403,'缺少活动ID参数');
        }
        else
        {
            $activityObj       = new IModel('activity');
            $this->activityRow = $activityObj->getObj('id = '.$this->activity_id);
            if(empty($this->activityRow))
            {
    ...
    function products()
    {
        ...
        //dyg_jzw 20160727 修正价格区间
        $product_info = $tb_product->getObj('goods_id='.$goods_id,'max(sell_price) as maxSellPrice ,max(market_price) as maxMarketPrice, min(sell_price) as minSellPrice ,min(market_price) as minMarketPrice');
        if(isset($product_info['maxSellPrice']) && $product_info['maxSellPrice'])
        {
            if ($product_info['minSellPrice'] != $product_info['maxSellPrice'])
            {
                $goods_info['sell_price']   = $product_info['minSellPrice']."-".$product_info['maxSellPrice'];
                $goods_info['market_price'] = $product_info['minMarketPrice']."-".$product_info['maxMarketPrice'];
            }
            else
            {
                $goods_info['sell_price']   = $product_info['minSellPrice'];
                $goods_info['market_price'] = $product_info['minMarketPrice'];
            }

        }

        //dyg_jzw 20170220 获取当前用户组的用户倍数
        $group_point_times = 1;
        ...
        if ($this->group_id)
        {
            ...
            $groupRow = $groupDB->getObj('id = '.$this->group_id);
        ...
        */
        ...
        if (count($products_info_all) > 0)
        {
            ...
            //存在多个规格的商品
            $group_price_arr = $countsumInstance->getGroupPrice($goods_id,'goods_arr');

            //dyg_jzw 20161219 获取高返后真正价格
            $group_real_price_arr = $countsumInstance->getGroupRealPrice($goods_id,'goods_arr');

            $goods_info['group_price_arr'] = array();

            //商品规格中插入价格
            foreach ($products_info_all as $_key => $_pro_spec)
            {
                $goods_info['group_price_arr'][$_key] = array(
                    'id' => $_pro_spec['id'],
                    'spec_array' => $_pro_spec['spec_array'],
                    'products_no' => $_pro_spec['products_no'],
                    'store_nums' => $_pro_spec['store_nums'],
                    'market_price' => $_pro_spec['market_price'],
                    'sell_price' => $_pro_spec['sell_price'],
                    'weight' => $_pro_spec['weight'],
                    'point'	=> intval($_pro_spec['point'] * $group_point_times), //dyg_jzw 20170220积分显示
                    'products_no' => $_pro_spec['products_no']
                );

                //dyg_jzw 20161219 是否存在高返返后真正价格
                if ($group_real_price_arr)
                {
                    foreach ($group_real_price_arr as $_real_price)
                    {
                        if ($_real_price['product_id'] == $_pro_spec['id'])
                        {
                            $goods_info['group_price_arr'][$_key]['real_price'] = $_real_price['real_price'];
                        }
                    }
                }
                ...
                if ($group_price_arr)
                {
                    foreach ($group_price_arr as $_pro_price)
                    {
                        if ($_pro_price['product_id'] == $_pro_spec['id'])
                        {
                            $goods_info['group_price_arr'][$_key]['group_price'] = $_pro_price['price'];
                        }
                    }
                }
                else
                {
                    $goods_info['group_price_arr'][$_key]['group_price'] = $_pro_spec['sell_price'];
                }

                //筛选出最大价, 最小价
                if (isset($goods_info['group_price_arr'][$_key]['group_price']))
                {
                    if ($goods_info['group_price_arr'][$_key]['group_price'] > $maxGroupPrice)
                    {
                        $maxGroupPrice = $goods_info['group_price_arr'][$_key]['group_price'];
                    }

                    if (empty($minGroupPrice) && $goods_info['group_price_arr'][$_key]['group_price'])
                    {
                        $minGroupPrice = $goods_info['group_price_arr'][$_key]['group_price'];
                    }

                    if ($goods_info['group_price_arr'][$_key]['group_price'] && $goods_info['group_price_arr'][$_key]['group_price'] < $minGroupPrice)
                    {
                        $minGroupPrice = $goods_info['group_price_arr'][$_key]['group_price'];
                    }
                }
            ...
            {
                $goods_info['group_price'] = $minGroupPrice . '-'.$maxGroupPrice;
            }
            else
            {
                $goods_info['group_price'] = $minGroupPrice;
            }
        }
        else
        {
            ...
            //dyg_jzw 20161219 单一规格商品的返后真正价格
            $goods_info['group_real_price'] = $countsumInstance->getGroupRealPrice($goods_id,'goods');

            //单一规格商品 积分*倍数
            $goods_info['point'] = intval($goods_info['point'] * $group_point_times);
        }
        ...
        if(IClient::getDevice() == 'mobile' && $goods_info['m_content'])
        {
            $goods_info['content'] = $goods_info['m_content'];
        }
    ...
    function comment_ajax()
    {
        ...
        //dyg_jzw 20151109 替换隐藏username
        if ($data)
        {
            foreach ($data as $_key => $_value)
            {
                $data[$_key]['username'] = substr_replace($_value['username'],'***',-4,3);
            }
        }
    ...
    function comments_list()
    {
        ...
        //评分级别
        $type_config = array('bad'=>'1,2','middle'=>'3','good'=>'4,5'); //dyg_jzw 20161123 修改4星也为好评 2星为差评
        $point       = isset($type_config[$type]) ? $type_config[$type] : "";

        //查询评价数据
        $this->commentQuery = Api::run('getListByGoods',$id,$point);

        //获取数据 dyg_jzw 20151109 替换隐藏username用
        $commentsList = $this->commentQuery->find();
        ...
        if ($commentsList)
        {
            foreach ($commentsList as $_key => $_value)
            {
                $commentsList[$_key]['username'] = substr_replace($_value['username'],'***',-4,3);
            }
        }
    ...
    public function comment_add()
    {
        ...
        if($re)
        {
            ...
            //dyg_jzw 20161128 删除该是否评价的缓存
            $_cache = new ICache('memcache');
            $_cache->del("getOrderComment_".$commentRow['order_no'].'_'.$commentRow['goods_id']);
    ...
    {
        //是否带默认防伪码查询
        $code = IFilter::act(IReq::get('c'));
        $code_for_makeup =    IFilter::act(IReq::get('c'));//好山好水化妆品防伪入口
        ...
        if (strlen($code) == 16)
        {
            $this->scode1 = substr($code, 0, 4);
            $this->scode2 = substr($code, 4, 4);
            $this->scode3 = substr($code, 8, 4);
            $this->scode4 = substr($code, 12, 4);
        }
    ...
    {
        //是否带默认防伪码查询
        $code = IFilter::act(IReq::get('h'));
        $this->code = $code;
        $this->redirect('scode2');
    }
    function scode_act2()
    {
        $scode    = IFilter::act(IReq::get('scode','post'));
        $code    = IFilter::act(IReq::get('code','post'));
        $captcha    = IFilter::act(IReq::get('captcha','post'));
        $_captcha   = ISafe::get('captcha');
        //pc版才需要输入验证码
        if(IClient::getDevice() == "pc" && (!$captcha || !$_captcha || $captcha != $_captcha))
        {
            $result = array(
                'success' => 0,
                'message' => '验证码错误，重新输入'
        ...
        {
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, "http://e.dyg.cn/security/get_result_json_ds23gjgj_34rjfnf_n211ob345jhghngvs/?h=".$code);
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
            curl_setopt($ch, CURLOPT_HEADER, 0);
            curl_setopt($ch, CURLOPT_POST, 1);
            curl_setopt($ch, CURLOPT_POSTFIELDS, array('code'=>$scode));//人工输入的scode
            $output = curl_exec($ch);
            curl_close($ch);
            $code_result = json_decode($output, true);
            //成功
            if($code_result['resultCode'] == 1)
            {
                $result = array(
                    'success' => 1,
                    'data' => $code_result['Result']
                );
            }
            else
            {
                $result = array(
                    'success' => 0,
                    'message' => '注意：'.$code_result['Content']
                );
            }
        }
        echo JSON::encode($result);
    ...
    function scode_act()
    {
        ...
        else
        {
            //获取防伪结果
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, "http://e.dyg.cn/security/get_result_json_ds23gjgj_34rjfnf_n211ob345jhghngvs/?c=".$scode);
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
            curl_setopt($ch, CURLOPT_HEADER, 0);
            ...
            $result = array(
                'success' => 1,
                'stat' => $code_result['stat'],
                'scode' => $scode,
                'product' => (isset($code_result['product_info']) && $code_result['product_info']) ? $code_result['product_info'] : '',
                'message' => $code_result['msg']
            );
        }
    ...
    //优惠券活动领取页面
    public function ticket_activity_page()
    {
        ...
        $sign = plugin::trigger('ticket_sign',$tid);
        $ticketDB = new IModel('ticket');
        $ticket = $ticketDB->getObj(' id = '.$tid.' AND is_open = 1','id,name,readme,start_time,end_time');
        if(!$ticket)
        {
            IError::show('找不到该券');
        }
        $data  = array(
            'tid'        => $ticket['id'],
            'sign'       => $sign,
            'readme'     => $ticket['readme'],
            'name'       => $ticket['name'],
            'start_time' => date('Y-m-d',strtotime($ticket['start_time'])),
            'end_time' => date('Y-m-d',strtotime($ticket['end_time'])),
        );
        $this->setRenderData($data);
        $this->redirect('ticket_activity_page');
    }
}
