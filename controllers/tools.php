...
class Tools extends IController implements adminAuthorization
{
	...
	function activity_edit()
	{
		...
		if($id)
		{
			...
			$activityRow = $activityObj->getObj('id = '.$id);

			if(!$activityRow)
			{
				IError::show(403,"活动信息不存在");
			}
			else
			{
				$activityRow['content'] = rawurldecode($activityRow['content']);
				$activityRow['content_mobile'] = rawurldecode($activityRow['content_mobile']);
			}
	...
	function activity_edit_act()
	{
		...
		$DataArray  = array(
			'title'       => IFilter::act(IReq::get('title','post')),
			//dyg_jzw 20160519 不过滤非法字符, 方便html代码
			'content'     => rawurlencode($_POST['content']),
			'content_mobile'     => rawurlencode($_POST['content_mobile']),
			'create_time' => ITime::getDateTime(),
			'keywords'    => IFilter::act(IReq::get('keywords','post')),
			'description' => IFilter::act(IReq::get('description','post'),'text'),
			'visibility'   => IFilter::act(IReq::get('visibility','post'),'int')
		...
		if($id)
		{
			//开始更新操作
			$where = 'id = '.$id;
			$is_success = $activityObj->update($where);
		}
		else
		{
			$id = $activityObj->add();
		...
		if(! $is_success)
		{
			$this->activityRow = $DataArray;
			$this->redirect('activity_edit',false);
			Util::showMessage('插入数据时发生错误');
		}
