(function autoValidate()
{
	...
	{
		var textfield = this;
		var outline = textfield.parentNode.parentNode;
		var input_parent = textfield.parentNode;
		...
		if((empty==null && value=='') || (value!='' && value.search(pattern) == -1))
		{
			...
			outline.className=outline.className.replace("invalid-text","");
			outline.className=outline.className.replace("valid-text","");
			if(outline.className.indexOf("invalid-text")==-1)outline.className += " invalid-text";
			msg=input_parent.nextSibling;
			...
			else if(alt!=null)
			{
				...
				input_parent.parentNode.insertBefore(new_msg,msg);
			}
		}
		else
		{
			...
			if(outline.className.indexOf("valid-text")==-1)outline.className +=" valid-text";

			/*if(outline.nextSibling.className.indexOf("invalid-msg") !== -1){
				msg=outline.nextSibling;
				while(msg && msg.nodeType==3)msg=msg.nextSibling;
				//删除错误的提示信息
				if(msg && (msg.tagName=='LABEL' || msg.tagName=='SPAN' ))
				{
			...
			if(msg && (msg.tagName=='LABEL' || msg.tagName=='SPAN' ))
			{
				if(empty!=null && value=='')
					...
					msg.className = "valid-msg no-alt";
				/*if(textfield.getAttribute('initmsg')==null) textfield.setAttribute('initmsg', msg.innerHTML);
			...
			else if(alt!=null)
			{
				...
				{
					new_msg.className = "valid-msg no-alt";
				}
			...
			if(this.type == 'password')
			{
				...
				else
				{
					for(var i=0; i<bind_arr_len; i++)
					{
						if(bind_arr[i].name == bind && bind_arr[i].value == this.value && bind_arr[i].value != '')
						{
							bind_arr[i].nextSibling.className = "valid-msg no-alt";
							bind_arr[i].className=textfield.className.replace("invalid-text","");
							if(bind_arr[i].className.indexOf("valid-text")==-1)this.className += ' valid-text no-alt';
							//bind_arr[i].nextSibling.innerHTML = '验证通过';
						}
					}
					msg.className = "valid-msg no-alt";
					textfield.className=textfield.className.replace("invalid-text","");
					if(textfield.className.indexOf("valid-text")==-1)this.className += ' valid-text no-alt';
					//msg.innerHTML = '验证通过';
					msg.innerHTML = '';
				}
			}
	...
	function validateOnSubmit()
	{
		...
		for(var i = 0; i < this.elements.length; i++)
		{
			...
			var outline = e.parentNode.parentNode;
			...
			if((e.type == "text" || e.type == "password" || e.type == "select-one" || e.type == "textarea") && e.getAttribute("pattern") && e.style.display!='none' && e.offsetWidth > 0)
			{
				...
				if (outline.className.indexOf("invalid-text")!=-1)
				{
					invalid = true;
