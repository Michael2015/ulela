		...
		$('[specId][specData]').bind('click',function()
		{
			...
			for(var _spec_index in goods_spec_array)
			{
				if (parseInt(_spec_index) > 0)
				{
					...
					{
						//规格值列表
						var _spec_val_arr = goods_spec_array[_spec_index].value.split(",");
						
						var _tmp_spec_arr = new Array();

						for (var i = 0; i < _spec_val_arr.length; i++) 
						{
							_tmp_spec_arr = specArray.concat();
							_tmp_spec_arr[_spec_index] = { 
															id: goods_spec_array[_spec_index].id,
										 					type: goods_spec_array[_spec_index].type,
										 					value: _spec_val_arr[i],
										 					name: goods_spec_array[_spec_index].name
										 				};
							//组合成key
							var _map_key = '';
							for(var _tk in _tmp_spec_arr)
							{
								if (parseInt(_tk) > 0 && _tmp_spec_arr[_tk] != null)
								{
									_map_key += ',' + _tmp_spec_arr[_tk].id + '@' + _tmp_spec_arr[_tk].value;
								}
							}

							//判断是否无库存
							//隐藏此按钮
							var value = "[specId='"+goods_spec_array[_spec_index].id+"'][specData='"+_spec_val_arr[i] + "']";

							if(typeof(spec_store_map[_map_key.substr(1)]) == 'undefined')
							{
								$(value).removeClass('current').addClass('disabled_spec').attr('disabled', true);
							}
							else
							{
								$(value).removeClass('disabled_spec').removeAttr('disabled');
							}
						}
			...
			{
				var _tmp_key = '';
				for (var _spec_index2 in specArray) 
				{
					if (parseInt(_spec_index2)> 0)
					{
						_tmp_key += ',' + specArray[_spec_index2].id + '@' + (specArray[_spec_index2].value);
					}
				}
