...
function orderFormClass()
{
	...
	{
		btDialog.open(creatUrl("block/address/id/"+addressId + "?is_cbe=" + is_cbe), //dyg_jzw 20160613 添加跨境电商地址
		{
			"id":"addressWindow",
			"title":"修改收货地址",
			"ok":function(){
	...
	{
		btDialog.open(creatUrl("block/address" + "?is_cbe=" + is_cbe), //dyg_jzw 20160613 添加跨境电商地址
		{
			"id":"addressWindow",
			"title":"添加收货地址",
			"ok":function(){
	...
	{
		btDialog.open(creatUrl("block/takeself"),{
			title:'选择自提点',
			okVal:'选择',
			ok:function()
			{
				...
				var takeselfJson = $(iframeWin).find('[name="takeselfItem"]:checked').val();
				if(!takeselfJson)
				{
					alert('请选择自提点');
					return false;
	...
	this.ticketShow = function()
	{
		...
		btDialog.open(creatUrl("block/ticket/sellerString/"+sellerArray.join("_")),{
			title:'选择代金券',
			okVal:'使用',
			ok:function()
			{
				...
				var formObject   = iframeWin.forms["ticketForm"];
				var resultTicket = 0;
