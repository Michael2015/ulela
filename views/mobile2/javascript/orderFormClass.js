...
function orderFormClass()
{
	...
	this.addressEdit = function(addressId, is_cbe) //dyg_jzw 20160613 添加跨境电商地址
	{
		layer.open({
			...
			yes: function(index, layero){
				...
				//var iframeWin = document.getElementById("addressWindow").contentDocument||document.frames["addressWindow"].document;
				...
				$.getJSON(formObject.action,$(formObject).serialize(),function(content){
					...
					$('input:radio[name="radio_address"]:first').parent('label').trigger('click');
	...
	{
		//如果删除的是已选中的地址，则自动选中下一个地址
		var is_checked = $('#address_'+addressId).is(':checked');
	...
	this.deliveryInit = function(defaultDeliveryId)
	{
		...
		{
			$('input:radio[name="delivery_id"][value="'+defaultDeliveryId+'"]').trigger('click');
		}
	}
	...
	this.ticketShow = function()
	{
		...
		layer.open({
			type: 2, //iframe层
			content: creatUrl("block/ticket/sellerString/"+sellerArray.join("_")), 
			id: "ticketShow",
			title: "选择代金券",
			area: ['350px', '450px'],
			btn: ['使用', '取消'],
	...
	this.ticketShow1 = function()
	{
		...
		var selectItem = null;
		//获取代金券信息
		$.getJSON(creatUrl("block/ticket_json/"),{"sellerString":sellerArray.join("_"), "direct_gid":direct_gid, "direct_type":direct_type, "direct_num":direct_num, "direct_active_id":direct_active_id , "direct_promo":direct_promo, "random":Math.random()}, function(json){
			if (json.success) {
				...
				for(var _tmp = 0; _tmp < selectItem.length; _tmp++) {
					if (selectItem[_tmp].can_use) {
						html += '<option value="'+selectItem[_tmp].id+'" seller="'+selectItem[_tmp].seller_id+'" price="'+selectItem[_tmp].value+'">减'+ selectItem[_tmp].value + '元：' + selectItem[_tmp].name+'</option>';
					} else {
						html += '<option value="'+selectItem[_tmp].id+'" seller="'+selectItem[_tmp].seller_id+'" price="'+selectItem[_tmp].value+'" disabled="disabled">(不可用)'+ selectItem[_tmp].name+'</option>';
					}
				}
