...
class _userInfo extends pluginBase
{
	...
	public function userRegAct()
	{
    	...
    	//邀请人用户名 dyg_jzw 20150618
        $inviter   = IFilter::act(IReq::get('inviter','post'));
    	...
    	//dyg_jzw 20160604 注册用户名为手机号
    	if(IValidate::mobi($username) == false)
        {
            return '注册手机号填写错误';
        }
        else if(! empty($inviter) && IValidate::mobi($inviter) == false)
        {
            return '邀请码填写错误';
        }
    	else
    	{
            ...
            if(!empty($inviter))
            {
                ...
                $inviterRow = $userObj->getObj($where);
		...
		$userArray = array(
			'username' => $username,
			'password' => md5($password),
			'inviter'  => $inviter //dyg_jzw 20150618
		);
	...
	public static function getUser()
	{
		...
		if($userRow = self::isValidUser($user['username'],$user['user_pwd']))
		{
			...
			$user['group_id']= $userRow['group_id']; //dyg_jzw 20161215 增加会员组获取
			return $user;
		}
			return null;
	}
	...
	public static function isValidUser($login_info,$password)
	{
		...
		//dyg_jzw 20160615 删除mobile，避免重复
		//$where   = "(u.username = '{$login_info}' or m.email = '{$login_info}' or m.mobile='{$login_info}') and m.status = 1 and u.id = m.user_id";
		$where   = "(u.username = '{$login_info}' or m.email = '{$login_info}') and m.status = 1 and u.id = m.user_id";
	...
	public function send_check_mail($email)
	{
		...
		$smtp   = new SendMail();
		$result = $smtp->send($email,"【{$siteConfig->name}】用户注册邮箱验证",$content);
		if($result===false)
		{
			IError::show(403,"发信失败,请重试！或者联系管理员查看邮件服务是否开启");
		}

		$message = "您的邮箱验证邮件已发送到{$email}！请到您的邮箱查收激活";
