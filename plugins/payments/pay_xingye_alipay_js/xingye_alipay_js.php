 ...
 * @author  yangmz
 */
class xingye_alipay_js extends paymentPlugin
{
    //支付插件名称
    public $name = '兴业银行支付宝支付JS版';
    ...
    public function getSendData($payment)
    {
        $return = array();
        //基本参数
        $return['service'] = 'pay.alipay.jspay';
        ...
        $return['buyer_logon_id'] = "13794888292";//买家支付宝账号,和buyer_ id 不能同时为空
        //除去待签名参数数组中的空值和签名参数
    ...
    {
        if($sendData['result_code'] ==0 && $sendData['status'] == 0)
        {
            $sendData['url'] = $sendData['pay_url'];
            include(dirname(__FILE__).'/template/pay.php');
        }
        else
        {
            $message =  '兴业银行支付宝JS接口失败';
            die($message);
        }
    }
